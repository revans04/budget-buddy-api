...
public class BudgetController : ControllerBase
{
  ...
  [AuthorizeFirebase]
  public async Task<IActionResult> UpdateImportedTransaction(string docId, string transactionId, [FromBody] ImportedTransaction updates)
  {
    try
    {
      updates.Id = transactionId;
      await _budgetService.UpdateImportedTransaction(docId, transactionId, updates);
      return Ok();
    }
    catch (Exception ex)
    {
  ...
  [AuthorizeFirebase]
  public async Task<IActionResult> GetImportedTransactionDocs()
  {
    try
    {
      var userId = HttpContext.Items["UserId"]?.ToString();
      var docs = await _budgetService.GetImportedTransactionDocs(userId);
      return Ok(docs);
    }
    catch (Exception ex)
    {
      Console.WriteLine($"Error in GetImportedTransactionDocs: {ex.Message}");
      return BadRequest(ex.Message);
    }
  }

  [HttpPost("shared/{sharedUid}")]
  [AuthorizeFirebase]
  public async Task<IActionResult> UpdateSharedBudgets(string sharedUid, [FromBody] List<string> budgetIds)
  {
    try
    {
      var ownerUid = HttpContext.Items["UserId"]?.ToString();
      await _budgetService.UpdateSharedBudgets(ownerUid, sharedUid, budgetIds);
      return Ok();
    }
    catch (Exception ex)
    {
      Console.WriteLine($"Error in UpdateSharedBudgets: {ex.Message}");
      return BadRequest(ex.Message);
    }
  }
}